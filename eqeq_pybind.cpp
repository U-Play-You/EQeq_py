...
extern double lambda;        // Coulomb scaling parameter
extern float hI0;            // electron affinity of hydrogen
extern bool useEwardSums;    // whether to use Ewald sums
extern bool isPeriodic;      // periodic or non-periodic
extern int mR;               // real-space range
extern int mK;               // reciprocal-space range
extern double eta;           // Ewald splitting parameter

extern void InitializeStringAtomLabelsEnumeration();
extern void LoadIonizationDataFromString(const std::string &data);
extern void LoadChargeCentersFromString(const std::string &text);
extern void LoadCIFFile(std::string filename);
extern void Qeq();
extern void RoundCharges(int digits);

extern const std::string ionization_data_text;
extern const std::string chargecenters_text;
